...
export async function generateMetadata({
  ...
  try {
    ...
    const description =
      ...
      `Read about the ${matterResult.data.title} project`;
    const imageUrl =
      matterResult.data.imageUrl || `${SITE_INFO.url}/images/og-image.png`;
...
const CaseStudyPage = async ({
  ...
  params: { work: string; order: number };
}) => {
  const fullPath = path.join(
    process.cwd(),
    "data",
  ...
  try {
    ...
    return (
      <div className="container mx-auto px-6 py-20 text-center">
        <h1 className="text-5xl font-bold text-[#4D4D4D] mb-4">
          Project Not Found
        </h1>
  ...
  return (
    <div className="container masonery mx-auto px-6 py-20">
      <h1 className="text-5xl font-bold text-center text-[#4D4D4D] mb-4">
        {project.title}
      </h1>

      {project.subtitle && (
        <h2 className="text-2xl text-center text-[#4D4D4D] mb-8">
          {project.subtitle}
        </h2>
      )}
      ...
      <div
        className="case-study prose mx-auto"
